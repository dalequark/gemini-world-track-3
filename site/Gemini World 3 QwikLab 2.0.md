...
> **A note on user identity.** The template gives each browser a stable `user_id` (a random id saved in `localStorage`) and sends it with every message, so each visitor gets their own agent session — and their own [Memory Bank](#adding-memory). For a real multi-user app, replace the `localStorage` id with your signed-in user's id.
